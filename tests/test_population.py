...
def test_population(tmpdir):
    ...
    p = p.filter_events(threshold=0, #observing_runs=["O3a", "O3b"],
                        filter_valid_mcz=False)
    fig, _ = p.plot_event_mcz_estimates()
    fig.savefig(
        f"{tmpdir}/event_mcz_estimates_only_o3.png", bbox_inches="tight", dpi=300
    )
    ...
    fig, _ = p.plot_event_mcz_estimates()
    fig.savefig(
        f"{tmpdir}/event_mcz_estimates_pastro_check.png", bbox_inches="tight", dpi=300
    )
    ...
    fig, _ = p.plot_event_mcz_estimates()
    fig.savefig(
        f"{tmpdir}/event_mcz_estimates_pastro_check_and_mcz_check.png", bbox_inches="tight", dpi=300
    )
