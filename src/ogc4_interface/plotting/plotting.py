...
def add_cntr(ax, X, Y, Z, color=CTOP):
    ax.contour(X, Y, gaussian_filter(Z, 1.2).T, levels=1, colors=color, linewidths=[0, 2], alpha=0.1)
...
def plot_event_mcz_uncertainty(data:pd.DataFrame, pass_fail=None):
    ...
    z, zup, zlow = data['redshift'], data['redshift_plus'], data['redshift_minus']
    mc, mcup, mclow = data['srcmchirp'], data['srcmchirp_plus'], data['srcmchirp_minus']
    names = data['Name']
    ...
    for i in range(n_events):
        ...
        axes[0].errorbar(z[i], y[i], xerr=zerr[i].reshape(-1,1), **kwgs, **clr)
        axes[1].errorbar(mc[i], y[i], xerr=mcerr[i].reshape(-1,1), **kwgs, **clr)
    ...
    axes[0].annotate('Events', xy=(0, 1), xytext=(-50, 5),
                xycoords=('axes fraction', 'axes fraction'),
                textcoords='offset points',
                ha='center', va='bottom', rotation=0,
                fontsize=12, fontweight='bold')
    axes[1].annotate('P(BBH)', xy=(1, 1), xytext=(+5, 5),
                     xycoords=('axes fraction', 'axes fraction'),
                     textcoords='offset points',
                     ha='center', va='bottom', rotation=0,
                     fontsize=12, fontweight='bold')
